...
const DropDownItem = ({ NavItem }: {NavItem:NavItem}) => { 
    return (
        <div className="relative group">
            ...
            <div className="hidden absolute p-4 top-  w-[300px] space-y-2 mt-2 rounded-md bg-white   left-0 group-hover:flex flex-col transition-opacity duration-200 delay-75">
                { 
                    NavItem.links.map((link, index) => (
                        <Link key={`link${index}`} href={ link.href} className="flex flex-col w-full ">
